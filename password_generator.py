...
def password(*args):
	...
	if length.get().isdigit() and int(length.get()) in range(4,21):
		p = choice(ascii_uppercase)+choice(ascii_lowercase)+choice(digits)+choice(punctuation)
		...
		while count < int(length.get()):
			...
			w += y
			count += 1
		if var.get():gen.set(p)
		else:gen.set(w)
	elif length.get().isdigit():
		messagebox.showwarning(message="Length should be in the range of 4 to 20")
	else:
...
len_entry.config(font=("verdana","12","bold"),foreground="blue")   # Change Fonts
len_entry.grid(row=1,column=2)

label=ttk.Label(frame,text="Enter Length ",relief="ridge")
label.config(font=("verdana","11","italic bold"),foreground="white",background="black")   
label.grid(row=1,column=1)

label1=ttk.Label(frame,textvariable=gen, wraplength="6.5c",relief="flat") 
label1.config(font=("verdana","12","bold italic"),foreground="red",background="white")     
